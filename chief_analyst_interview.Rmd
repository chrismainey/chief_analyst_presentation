...
__Regression based models:__

+ Poisson regression `r Citep(bib, "poissonRecherchesProbabiliteJugements1837")` (`R`) as basis for modelling
+ Mixture/compound distribution models
+ Multi-level models
...
+ Extraction into reporting tables as 'counts per day'
 + HES isn't per day, it's episodes.  So per day, is a spell happingin?
 + Investiagetd different methods of bed-day calculation, depeing on which you chose, you can call day cases zero time in hospital.
---
...
Mis-specified systematic component of a model
Presence of outliers
Variation between response probabilities (heterogeneity)


In models with fixed variance assumption (e.g. Poisson & Binomial), 
...
+ Commonly measures are:
  ...
  + [Dr Foster Hospital Standardised Mortality Ratio (HSMR)](https://www.nhs.uk/NHSEngland/Hospitalmortalityrates/Documents/090424%20MS%28H%29%20-%20NHS%20Choices%20HSMR%20Publication%20-%20Presentation%20-%20Annex%20C.pdf)


.pull-left[
+ Commonly make the cross-sectional comparisons with Funnel Plot

+ __Unanswered question: is trust X 'getting worse'?__

]

.pull-right[

```{r funnel, echo=FALSE, message=FALSE, warning=FALSE}
library(ragg)
library(COUNT)
library(FunnelPlotR)
library(dplyr)
...
This has resource implications - requiring clinical review and often highlighing data quality or statistical issues.
